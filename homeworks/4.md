...
Wolfram Alpha had a stroke calculating the above so here's the Python code I used where I chanced `summand(n)` for every $P$ and between $u$ and $v$ (this is snippet calculates $v$ for $P_-$ which I did last):
