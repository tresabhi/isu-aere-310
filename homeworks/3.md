...
It shouldn't be surprising that $\Gamma$ values across both parts of the question match since they both literally equal each other in the integral form. It also makes sense that $\Gamma_A = 0$ since it's symmetrical so there's no "spinny bits." It also makes sense that $\Gamma_B \neq 0$ since there's "shear" in the flow.
